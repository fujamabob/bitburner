import { NS } from "@ns";
import { get_server_list, get_server_path, al, return_all } from "lib/scan";
import { has_root, needs_root } from "./lib/root";
...
export async function main(ns: NS): Promise<void> {
    ...
    if (flags.p) {
        var path = 'home'
        for (let name of get_server_path(ns, args[0].toString())) {
            path += `; connect ${name}`
        }
        ns.tprint(path)
        return
    ...
    for (let name of get_server_list(ns, "home", where)) {
