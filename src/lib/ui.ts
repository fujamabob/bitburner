...
export namespace Formatter {
    export function number(n: number, fractionalDigits?: number, suffixStart?: number, isInteger?: boolean) {
        if (Number.isNaN(n))
            return "NaN" // I know, I know.  Tracebacks, though.
        return ns.formatNumber(n, fractionalDigits, suffixStart, isInteger)
    }

    export function percent(n: number, fractionalDigits?: number, suffixStart?: number) {
        if (Number.isNaN(n))
            return "NaN" // I know, I know.  Tracebacks, though.
        return ns.formatPercent(n, fractionalDigits, suffixStart)
    }

    export function ram(n: number, fractionalDigits?: number) {
        if (Number.isNaN(n))
            return "NaN" // I know, I know.  Tracebacks, though.
        return ns.formatRam(n, fractionalDigits)
    }

    export function time(milliseconds: number, milliPrecision?: boolean) {
...
printRaw(node) 	Prints a ReactNode to the script logs.
    */
    ...
    print_error(...args: any[]) {
        ns.print("ERROR", ...args)
    }

    print_warning(...args: any[]) {
    ...
    disable_ns_log(fn: string = "ALL") {
        ns.disableLog(fn)
    }

    enable_ns_log(fn: string = "ALL") {
        ns.enableLog(fn)
    }

    is_ns_log_enabled(fn: string = "ALL"): boolean {
