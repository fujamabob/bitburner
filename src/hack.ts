...
import { recommend_hack } from "./lib/hack";

export async function main(ns: NS) {
    const arg_schema = [
        ['i', Infinity],  // iterations before exiting
        ['t', 0.5],   // % of max money necessary
    ...
    if (flags.r) {
        const recommendation = recommend_hack(server, flags.t)
        ns.tprint(recommendation.describe())
        return
    }

    for (let i = 0; i < flags.i; i++) {
        if (!flags.f && !recommend_hack(server, flags.t).recommend)
            break;
